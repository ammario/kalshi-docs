...
Historical replay through `EventResendRequest` (`35=U1`) on both `KalshiNR` and `KalshiDC` requires [account allowlisting](/fix-margin/drop-copy). NR users can log in and trade without replay access.
